...
function contained(e, t) {
  ...
  return (
    s.scriptId === e.scriptId &&
    !(e.lineNumber < s.lineNumber || e.lineNumber > i.lineNumber) &&
    !(e.lineNumber === s.lineNumber && e.columnNumber < s.columnNumber) &&
...
export class DebuggerModel extends SDKModel {
  constructor(e) {
    ...
    e.registerDebuggerDispatcher(new DebuggerDispatcher(this));
      ...
      e.suspended() || this._enableDebugger(),
  ...
  async _disableDebugger() {
    ...
    (this._debuggerEnabled = !1),
      ...
      "string" == typeof this._debuggerId &&
        _debuggerIdToModel.delete(this._debuggerId);
  }
  _skipAllPauses(e) {
    this._skipAllPausesTimeout &&
      (clearTimeout(this._skipAllPausesTimeout),
      (this._skipAllPausesTimeout = 0)),
      this._agent.invoke_setSkipAllPauses({ skip: e });
  }
  skipAllPausesUntilReloadOrTimeout(e) {
    this._skipAllPausesTimeout && clearTimeout(this._skipAllPausesTimeout),
      this._agent.invoke_setSkipAllPauses({ skip: !0 }),
      (this._skipAllPausesTimeout = setTimeout(
        this._skipAllPauses.bind(this, !1),
        e
      ));
  }
  ...
  async _computeAutoStepSkipList() {
    ...
    if (e) {
      const t = this._debuggerPausedDetails.callFrames[0].location(),
        s = await e.rawLocationToUILocation(t);
      if (s) {
        ...
        if (i) return i.filter((e) => contained(t, e));
      }
    }
    return [];
  }
  async stepInto() {
    (this._autoStepSkipList = await this._computeAutoStepSkipList()),
      this._agent.invoke_stepInto({ breakOnAsyncCall: !1 });
  }
  async stepOver() {
    (this._autoStepOver = !0),
      (this._autoStepSkipList = await this._computeAutoStepSkipList()),
  ...
  scheduleStepIntoAsync() {
    this._agent.invoke_stepInto({ breakOnAsyncCall: !0 });
  }
  resume() {
    this._agent.invoke_resume({ terminateOnResume: !1 }),
      (this._isPausing = !1);
  }
  pause() {
    (this._isPausing = !0), this._skipAllPauses(!1), this._agent.invoke_pause();
  }
  _pauseOnAsyncCall(e) {
  ...
  async setBreakpointByURL(e, t, s, i) {
    ...
    for (let e = 0, s = o.length; e < s; ++e) {
      const s = o[e];
      t === s.lineOffset &&
        (a = a ? Math.min(a, s.columnOffset) : s.columnOffset);
    }
    s = Math.max(s || 0, a);
    ...
    if (n.getError()) return { locations: [], breakpointId: null };
    let c = [];
    return (
      n.locations &&
        (c = n.locations.map((e) => Location.fromPayload(this, e))),
      { locations: c, breakpointId: n.breakpointId }
    );
  }
  async setBreakpointInAnonymousScript(e, t, s, i, r) {
    const a = await this._agent.invoke_setBreakpointByUrl({
      ...
      o = a.getError();
    if (o)
      return "Either url or urlRegex must be specified." !== o
    ...
    return (
      ...
      { locations: n, breakpointId: a.breakpointId }
    );
  }
  async _setBreakpointBySourceId(e, t, s, i) {
    ...
    if (r.getError()) return { breakpointId: null, locations: [] };
    let a = [];
    return (
      r.actualLocation && (a = [Location.fromPayload(this, r.actualLocation)]),
      { locations: a, breakpointId: r.breakpointId }
    );
  }
  ...
  _reset() {
    ...
    if (e) for (const t of this._scripts.values()) e.removeScript(t);
  ...
  scriptsForExecutionContext(e) {
    ...
    for (const s of this._scripts.values())
      s.executionContextId === e.id && t.push(s);
    return t;
  }
  setScriptSource(e, t, s) {
    const i = this._scripts.get(e);
    i && i.editSource(t, this._didEditScriptSource.bind(this, e, t, s));
  }
  ...
  async _setDebuggerPausedDetails(e) {
    if (((this._isPausing = !1), (this._debuggerPausedDetails = e), e)) {
      const t = e.callFrames[0].location();
      for (const e of this._autoStepSkipList) if (contained(t, e)) return !1;
      if (
    ...
    } else this.setSelectedCallFrame(null);
    return !0;
  }
  ...
  async _pausedScript(e, t, s, i, r, a, o) {
    if (o) {
      ...
      return await Promise.all(e), void this.resume();
    }
    ...
    if (this._continueToLocationCallback) {
      ...
      if (((this._continueToLocationCallback = null), e(n))) return;
    }
    (await this._setDebuggerPausedDetails(n)) ||
      (this._autoStepOver
        ? this._agent.invoke_stepOver({})
        : this._agent.invoke_stepInto({ breakOnAsyncCall: !1 })),
  ...
  _parsedScriptSource(e, t, s, i, r, a, o, n, c, l, u, d, p, g, h, m, _, b, S) {
    ...
    if (y) return y;
    let k = !1;
    c && "isDefault" in c && (k = !c.isDefault), (t = this._internString(t));
    const C = new Script(
      this,
      e,
      t,
    ...
    const f = Bindings.DebuggerWorkspaceBinding.instance().getLanguagePluginManager(this);
    if (
      ...
      if (e && !p) {
        ...
        t && this._sourceMapManager.detachSourceMap(t),
          this._sourceMapIdToScript.set(e, C),
          this._sourceMapManager.attachSourceMap(
            C,
            C.sourceURL,
            C.sourceMapURL
          );
  ...
  setSourceMapURL(e, t) {
    ...
    s &&
      this._sourceMapIdToScript.get(s) === e &&
  ...
  _unregisterScript(e) {
    console.assert(e.isAnonymousScript()), this._scripts.delete(e.scriptId);
  }
  _collectDiscardedScripts() {
    if (this._discardableScripts.length < 1e3) return;
  ...
  createRawLocationByURL(e, t, s) {
    ...
    for (let e = 0, a = r.length; e < a; ++e) {
      ...
      if (
        ...
        !(
          a.lineOffset > t ||
          (a.lineOffset === t && a.columnOffset > s) ||
          a.endLine < t ||
          (a.endLine === t && a.endColumn <= s)
        ))
      ) {
        i = a;
        break;
  ...
  createRawLocationsByStackTrace(e) {
    ...
    let s = e;
    for (; s; ) {
      for (const e of s.callFrames) t.push(e);
      s = s.parent;
    }
    const i = [];
    for (const e of t) {
      ...
      function (e) {
        ...
        if (e.internalProperties)
          for (const s of e.internalProperties)
            "[[FunctionLocation]]" === s.name && (t = s.value);
        let s = null;
        if (e.properties)
          for (const t of e.properties)
            if (
              ("name" === t.name &&
                ...
                "string" === t.value.type &&
                (s = t.value),
              "displayName" === t.name && t.value && "string" === t.value.type)
            ) {
              s = t.value;
              break;
            }
  ...
  async setBlackboxPatterns(e) {
    ...
    return t && console.error(t), !t;
  }
  dispose() {
    this._sourceMapManager.dispose(),
      this._debuggerId && _debuggerIdToModel.delete(this._debuggerId),
      Common.Settings.Settings.instance()
        .moduleSetting("pauseOnExceptionEnabled")
  ...
  _internString(e) {
    ...
    return void 0 === t ? (this._stringMap.set(e, e), e) : t;
  }
}
...
class DebuggerDispatcher {
  ...
  paused({
    ...
    asyncStackTrace: r,
    asyncStackTraceId: a,
    asyncCallStackTraceId: o,
  }) {
    this._debuggerModel._pausedScript(e, t, s, i || [], r, a, o);
  }
  resumed() {
  ...
  breakpointResolved({ breakpointId: e, location: t }) {
    this._debuggerModel._breakpointResolved(e, t);
  }
}
...
export class CallFrame {
  constructor(e, t, s) {
    ...
    for (let e = 0; e < s.scopeChain.length; ++e) {
      ...
      this._scopeChain.push(t),
        t.type() === Protocol.Debugger.ScopeType.Local &&
          (this._localScope = t);
    }
    s.functionLocation &&
  ...
  static fromPayloadArray(e, t) {
    ...
    for (let i = 0; i < t.length; ++i) {
      const r = t[i],
        a = e.scriptForId(r.location.scriptId);
      a && s.push(new CallFrame(e, a, r));
    }
    return s;
  }
  get script() {
  ...
  async setReturnValue(e) {
    if (!this._returnValue) return null;
  ...
  async evaluate(e) {
    ...
    if (
      (!!e.throwOnSideEffect || void 0 !== e.timeout) &&
      (!1 === t.hasSideEffectSupport() ||
        (null === t.hasSideEffectSupport() &&
  ...
  async restart() {
    ...
    ).getError() || this.debuggerModel.stepInto();
  }
}
export class Scope {
  constructor(e, t) {
...
export class DebuggerPausedDetails {
  constructor(e, t, s, i, r, a, o) {
    (this.debuggerModel = e),
      ...
      a && (this.asyncStackTrace = this._cleanRedundantFrames(a)),
  ...
  _cleanRedundantFrames(e) {
    ...
    for (; t; )
      "async function" === t.description &&
        t.callFrames.length &&
        t.callFrames.shift(),
        s && !t.callFrames.length ? (s.parent = t.parent) : (s = t),
        (t = t.parent);
    return e;
  }
}
SDKModel.register(DebuggerModel, Capability.JS, !0);
